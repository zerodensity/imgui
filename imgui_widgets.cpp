...
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx2(label, NULL, buf, (int)buf_size, precision, ImVec2(0, 0), flags, callback, user_data);
}
...
bool ImGui::InputTextEx2(const char* label, const char* hint, char* buf, int buf_size, int precision, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            state->Stb->cursor = 6;
            if ((ImGui::IsKeyDown(ImGuiKey_LeftShift) || ImGui::IsKeyDown(ImGuiKey_RightShift)) && (ImGui::IsKeyDown(ImGuiKey_LeftAlt) || ImGui::IsKeyDown(ImGuiKey_RightAlt)))
            {
                if (precision >= 4)
                    state->Stb->cursor = 8;
            }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            {
                ImVec2 tl = cursor_screen_rect.GetTL(); tl.x -= 1; tl.y -= 2;
                ImVec2 br = cursor_screen_rect.GetBR();
                ImVec2 bl = cursor_screen_rect.GetBL(); bl.x -= 1; bl.y += 2;
                ImU32 color = ImGui::ColorConvertFloat4ToU32(ImVec4(1, 1, 1, 1));
                draw_window->DrawList->AddLine(tl, ImVec2(tl.x + 6, tl.y), color);
                draw_window->DrawList->AddLine(bl, ImVec2(bl.x + 6, bl.y), color);
                //draw_window->DrawList->AddRect(tl, br, color);
            }
